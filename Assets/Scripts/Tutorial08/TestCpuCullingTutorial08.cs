...
public class TestCpuCullingTutorial08 : MonoBehaviour
{
    public int Count;
    public int Radius;
    ...
    private List<(int,int)> index1List = new List<(int,int)>();
    ...
    void UpdateIndexAndCount()
    {
        ...
        for (int i = 0; i < this.meshFilters.Count; i++)
        {
            ...
            if (IsVisible(this.meshFilters[i].transform.position, MeshInfoList[tIndex]))
            {
                ...
                index1List.Add((tIndex,i));
            }
        }

        index1List.Sort((t1, t2) => t1.Item1 - t2.Item1);

        for (int i = 0; i < index1List.Count; i++)
        {
            matrixIndexData[i] = index1List[i].Item2;
        }
        TotalVisibleCount = index1List.Count;
